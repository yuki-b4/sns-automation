# ファネル設計：A案「自責⇔他責ループ解体」言語化ファネル

最終更新: 2026-05-09

> 本ドキュメントは SNS（Threads／無料note）→ 公式LINE → 無料webセミナー → 個別相談会 → バックエンド講座（愛を深め続けるマインド構築講座 ¥550,000）の販売動線の単一ソースである。各ステップの目的・コンテンツ要件・KPI・計測スキーマを集約する。
>
> ファネルコンセプトの選定経緯と他案の比較は本ファイル末尾の「採用経緯」を参照。

---

## 1. 全体概要

| 項目 | 内容 |
|---|---|
| 採用コンセプト | A案「自責⇔他責ループ解体」言語化ファネル |
| 主チャネル | 公式LINE（新規立ち上げ） |
| 想定リードタイム | 約1ヶ月（LINE登録から成約まで） |
| 入口フック | 「『自分のせい』と『タイミングのせい』を行ったり来たりして、次に何を変えればいいか分からない」（`config/strategy.json` `persona.pain_points` #2 直撃） |
| バックエンド | 愛を深め続けるマインド構築講座 ¥550,000 |
| 個別相談 | 45分・無料（通常 ¥50,000/h相当 ＝ 45分換算 ¥37,500相当の希少性） |

### ファネル構造

```
SNS（Threads／無料note）
   ↓ [認知/共感/興味] 投稿で内側パターンの存在を可視化
公式LINE登録（特典：内側パターン書き出しワークPDF）
   ↓ [興味→理解] 7日間ステップ配信で構造を理解
無料webセミナー（90分）
   ↓ [理解] 3つの典型パターンと内側設計を体系的に解説
個別相談会（45分・無料）
   ↓ [納得] 自分のパターンを発信者と言語化／講座適合判定
バックエンド契約
```

### ペルソナペインとの対応

| ステップ | 直撃するペイン（`persona.pain_points` 番号） | 提供する転換 |
|---|---|---|
| SNS | #1 別れ際の言葉が脳裏によぎる ／ #3 知識はあるが応用できない | 「同じ反復パターンを抱える人がここにいる」と気づく |
| 公式LINE登録 | #2 自責⇔他責ループから抜けられない | 自分の内側パターンを書き出す体験を提供 |
| LINE配信 | #3 知識を自分に応用できない ／ #4 テクニックでは解けないと気づいている | 内側設計の構造を順を追って学ぶ |
| 無料セミナー | #2 ループから抜けられない ／ #5 踏み込みと見極めで動けない | 3つの典型パターンと脱却設計を体系で受け取る |
| 個別相談 | #7 友人・仕事仲間に相談できない孤独 | 発信者と1対1で自分のパターンを言語化する |
| バックエンド | （契約後）pain全体への継続的アプローチ | 講座でマインド書き換えを実装 |

---

## 2. 各ステップ詳細設計

### 2.1 SNS層（Threads／無料note）

**目的**: ペルソナに「同じ反復パターンを抱えている自分」を可視化し、公式LINE登録への動機を作る。

**役割**（`config/strategy.json` `funnel.sns_role` に準拠）:
- 認知／共感／興味の3段階を担う
- 最大KPIは公式LINE登録への遷移
- バックエンド／セミナー／個別相談を**SNS本文で直接訴求しない**（`funnel.backend_path` 制約）

**主要投稿タイプの活用**: 既存の `config/strategy.json` `post_types` および `post_rotation` をそのまま使用する。本ファネル特有の変更は次の動線設計のみ。

**動線設計**:
- Threadsプロフィール → 公式LINE登録URL（短縮URL推奨：クリック計測のため）
- noteプロフィール → 同上
- 投稿本文内では公式LINEリンクを貼らない（`backend_path` の "SNSから直接誘導しない" の精神を保持し、プロフィール経由のみで誘導）

### 2.2 公式LINE登録（入口）

**目的**: 興味段階の読者を理解段階へ進める起点。継続接点を確立する。

**登録特典**: 「自分の内側パターン書き出しワーク」PDF
- 8〜12ページ程度
- ペルソナの自責⇔他責ループを言語化するための問い10個＋ワークシート
- 単独で完結する価値があり、登録直後に自動応答で配信

**LINE上の会話設計**:
- 登録直後: ウェルカムメッセージ＋特典PDF配信＋7日間配信の予告
- ステップ配信完了後: 個別問い合わせ可能な状態を維持（読者の質問に発信者が手動返信）
- セミナー直前: リマインド配信
- セミナー直後: 個別相談申込導線

### 2.3 LINEステップ配信（7日間）

| Day | テーマ | 目的（ファネル段階） | 必須要素 |
|---|---|---|---|
| Day 0（登録直後） | ウェルカム＋特典PDF＋自己紹介 | 興味 | 7日間配信予告／発信者の立場明示 |
| Day 1 | 「同じパターンを反復している自分」の言語化 | 興味 | 反復パターンの典型例（許可系の延長） |
| Day 2 | 自責⇔他責ループの正体（脳科学的メカニズム） | 興味→理解 | 脳科学根拠1つ／構造図1つ |
| Day 3 | クライアント事例1（優しくして苦しくなった人） | 理解 | 観察者立場（「クライアントの〇〇さん」形式・`docs/author_profile.md` ルール準拠） |
| Day 4 | なぜテクニック型では解けないのか | 理解 | 差別化軸の言語化（`positioning.differentiation` 注入） |
| Day 5 | 発信者の脱却プロセス（BEFORE-AFTER） | 理解 | 過去の自分の反復パターン／妻との関係構築までの過程（`positioning.credibility` 準拠） |
| Day 6 | 3つの典型パターン概要 | 理解→セミナー期待醸成 | セミナーで詳細を語る予告／パターン名のみ提示 |
| Day 7 | 無料webセミナー案内 | セミナー予約 | 開催日（毎月1〜2回）／個別相談（通常¥50,000/h相当が45分無料）の希少性訴求 |

**設計ルール**:
- 各日の配信は500〜1500文字程度。LINEで読みやすい改行を意識する
- 心理学・脳科学の根拠は「メカニズム説明」として組み込み、出典明記による権威誇示はしない
- Day 5の発信者開示は「一度で解決した成功譚」にしない（`post_types.personal` の "同じ罠を何度もくぐり抜けてきた当事者" ルール準拠）
- Day 7のセミナー告知は「次回開催を逃すと次は◯日後」と明示し、希少性を演出する

### 2.4 無料webセミナー（90分）

**目的**: 理解段階を完成させ、納得段階への入口（個別相談）を提示する。

**開催頻度**: 月1〜2回。LINE登録から最大3週間以内に出席可能とする（リードタイム1ヶ月維持のため）。

**形式**: Zoom等のオンライン双方向。録画案内はしない（出席のインセンティブを保つ）。

**章立て案**:

| 時間 | セクション | 内容 |
|---|---|---|
| 0-10分 | オープニング | 自己紹介／本日のゴール／参加者への問いかけ（チャット参加） |
| 10-30分 | 自責⇔他責ループの全体構造 | LINE配信内容を1枚スライドで体系化 |
| 30-60分 | 3つの典型パターンと内側設計 | パターンA/B/Cそれぞれにクライアント事例＋脱却ステップ |
| 60-75分 | 発信者の脱却プロセス詳細 | BEFORE-AFTER／妻との関係性で実装したマインド設計 |
| 75-85分 | Q&A | チャット質問に回答 |
| 85-90分 | 個別相談会の案内 | 45分・無料／通常¥50,000/h相当／参加者限定枠／申込フォーム提示 |

**設計ルール**:
- スライドは20枚以内
- 「3つの典型パターン」のネーミングは差別化軸の言語化（テクニック名ではなく内側構造の名前）に揃える
- 個別相談の希少性訴求は「申込フォームURLは本日中限定／今月の枠は◯名まで」のような形でクロージング設計を入れる

### 2.5 個別相談会（45分・無料）

**目的**: 納得段階を完成させ、バックエンド契約へクロージングする。

**料金ポジショニング**:
- 通常 ¥50,000/h相当のコーチング枠の45分版（¥37,500相当）
- 本ファネル参加者は無料。LINE特典＋セミナー出席を経た読者にのみ案内

**進行設計案**:

| 時間 | セクション | 目的 |
|---|---|---|
| 0-5分 | アイスブレイク／本日のゴール共有 | 信頼関係構築 |
| 5-20分 | 現状ヒアリング | 恋愛経験／反復パターン／いまの苦しさを聴く |
| 20-35分 | 言語化セッション | 発信者と一緒に内側パターンを文字にする（特典PDFワークの深化） |
| 35-40分 | カスタムロードマップ提示 | 「あなたの場合、こう書き換えると」の方向性 |
| 40-45分 | 講座の概要提示／検討期間の提示 | バックエンドの構造／検討期間（例：48時間以内に返答）を提示 |

**設計ルール**:
- 押し売りしない。納得段階に到達していなければ無理に契約させない
- 講座を提案する判断基準：参加者が「自分の内側パターンが分かった」と言語化できる状態に到達したか
- 検討期間中はLINEで質問対応する。期間内返答がなければ自動キャンセル

### 2.6 バックエンド契約

`config/strategy.json` `positioning.backend_product` の通り。本ドキュメントでは契約後の流れは扱わない。

---

## 3. KPI設計（4層分離）

層を独立に評価することで、改善対象を特定できる構造にする。

### 3.1 層の定義と目標値

```
【層1：SNS到達層】
   インプレッション → 到達者 → プロフィール訪問 → リンククリック
                  ↓
【層2：ファネル入口層】← CVR目標 30%
   リンククリック → LINE登録
                  ↓
【層3：ナーチャリング層】← CVR目標 50%
   LINE登録 → セミナー予約 → セミナー出席
                  ↓
【層4：セールス層】← CVR目標 50% / 50%
   セミナー出席 → 個別相談申込 → 個別相談実施 → 成約
```

### 3.2 各指標の分母／分子と目標値

| 層 | 指標 | 分母（比較元） | 分子 | 目標 | 計測ソース |
|---|---|---|---|---|---|
| 1-a | インプレッション→到達者比 | 月間インプレッション | 月間ユニーク到達者 | 参考値 | Threads Insights |
| 1-b | 到達者→プロフィール訪問率 | 月間ユニーク到達者 | 月間プロフィール訪問 | 参考値（5〜10%） | Threads Insights |
| 1-c | プロフィール訪問→リンククリック率 | 月間プロフィール訪問 | 月間リンククリック | 10〜15% | Threads Insights |
| **2** | **リンククリック→LINE登録率** | **月間リンククリック** | **月間LINE登録** | **30%** | LINE公式アカウント管理画面＋短縮URL |
| **3-a** | **LINE登録→セミナー予約率** | **コホート別LINE登録数** | **コホート別セミナー予約数** | **50%** | LINE側フォーム or Googleフォーム |
| 3-b | セミナー予約→出席率 | セミナー予約数 | 実出席数 | 70〜80%（業界相場） | Zoom等セミナーツール |
| **4-a** | **セミナー出席→個別相談申込率** | **セミナー出席者数** | **個別相談申込数** | **50%** | セミナー後フォーム |
| 4-b | 個別相談申込→実施率 | 個別相談申込数 | 個別相談実施数 | 80〜90%（業界相場） | カレンダー予約管理 |
| **4-c** | **個別相談実施→成約率** | **個別相談実施数** | **バックエンド成約数** | **50%** | 手動記録 |

**太字** が運用者提示の主要4目標値（30 / 50 / 50 / 50）。それ以外は補助KPI。

### 3.3 補助KPI

| 補助指標 | 計算 | 目的 |
|---|---|---|
| 総合CVR（リンククリック→成約） | 30% × 50% × 50% × 50% = 3.75% | ファネル全体効率の単一スコア |
| 必要トラフィック（月1件成約） | 月8件LINE登録 ／ 月27件リンククリック | SNS集客の月次ノルマ算出 |
| 必要トラフィック（月3件成約） | 月24件LINE登録 ／ 月80件リンククリック | スケール時の集客ノルマ |
| 平均LTV／顧客 | バックエンド ¥550,000 ＋ ミドルエンド購入額（任意） | 1成約あたりの売上換算 |
| コホート別追跡 | LINE登録月別に層3〜4を集計 | ナーチャリング配信改善の効果検証 |

### 3.4 設計ポリシー

1. **層を独立に評価する**：層1が悪化したら投稿コンテンツ／プロフィールを改善対象に、層2が悪化したらLINE登録ページの誘導文／登録特典を改善対象にする。層を混ぜると改善対象が特定できない。
2. **層3〜4はコホート（LINE登録月）で集計する**：単純月次比だと先月登録者が今月セミナーに来る現象でCVRが歪む。1ヶ月リードタイムなので、登録月+1ヶ月で層3〜4のCVRが確定する。
3. **層1と層2の境目（リンククリック）の計測精度を最優先する**：ファネル全体の最大ボトルネックは、ほぼ確実にこの境目。Threads Insights のリンククリック数は粒度が粗いため、Bitly等の短縮URLで独立計測する。

---

## 4. ファネルKPIシート スキーマ設計

Google Sheets を `config/strategy.json` 周辺と同じ DBレイヤーで運用する。3シート構成。

### 4.1 シート構成

| シート名 | 役割 | 更新頻度 | 更新主体 |
|---|---|---|---|
| `ファネル登録ログ` | 1人=1行のソース・オブ・トゥルース | LINE登録／予約／実施／成約イベント発生時 | 運用者（LINE管理画面・フォームから手動転記） |
| `ファネルKPI月次` | 運用ダッシュボード（層1〜2＋当月実施イベント） | 月初に前月分集計 | 半自動（層1は `collect_metrics.py` 拡張、それ以外は登録ログから集計式） |
| `ファネルKPIコホート` | コホート分析（層3〜4・LINE登録月起点） | 月次（前々月コホートの30日／60日成績が確定する月初） | 登録ログから集計式 |

### 4.2 シートA：`ファネル登録ログ`（ソース・オブ・トゥルース）

1人のLINE登録者につき1行。各イベントが発生するたびに該当列を埋める。

| 列名 | 型 | 説明 |
|---|---|---|
| `registered_at` | datetime (YYYY-MM-DD HH:MM) | LINE登録日時 |
| `cohort_month` | text (YYYY-MM) | 登録月（コホート集計の主キー） |
| `handle_or_alias` | text | LINE上の表示名／本人合意のうえで記録 |
| `registration_source` | text | 流入元（threads / note / other） |
| `source_post_id` | text | 流入元投稿ID（特定できる場合のみ） |
| `seminar_session_id` | text | 出席（予約）したセミナー回（YYYY-MM-DD形式の開催日） |
| `seminar_reserved_at` | datetime | セミナー予約日時 |
| `seminar_attended` | bool (TRUE/FALSE) | 実出席したか |
| `consultation_requested_at` | datetime | 個別相談申込日時 |
| `consultation_implemented_at` | datetime | 個別相談実施日時 |
| `consultation_status` | text | planned / done / no_show / canceled |
| `contract_at` | date | バックエンド契約日 |
| `contract_amount` | int | 契約金額（¥） |
| `midend_purchase_at` | date | ミドルエンド購入日（任意） |
| `midend_amount` | int | ミドルエンド購入金額（¥） |
| `status` | text | registered / nurturing / seminar_attended / consultation_done / contracted / lost / cooling |
| `notes` | text | 自由記述 |

**運用ルール**:
- 個人を識別するLINEユーザーIDは記録しない（プライバシー配慮）。`handle_or_alias` でユニーク識別する
- 同一人物の重複行が出ないよう、登録日時＋ハンドルで一意性を保つ
- `status` は最新状態を上書きする。タイムラインは各 `*_at` 列で復元できる

### 4.3 シートB：`ファネルKPI月次`（運用ダッシュボード）

1ヶ月＝1行。月初に前月分を集計。

| 列名 | 型 | 算出元 |
|---|---|---|
| `year_month` | text (YYYY-MM) | — |
| `impressions` | int | Threads Insights（`collect_metrics.py` 拡張） |
| `reach_unique` | int | Threads Insights |
| `profile_visits` | int | Threads Insights |
| `link_clicks` | int | 短縮URL管理画面 or Threads Insights |
| `line_registrations` | int | `ファネル登録ログ` の `registered_at` が当月の行数 |
| `seminar_sessions_count` | int | 当月開催したセミナー回数 |
| `seminar_attendances_total` | int | `seminar_attended=TRUE` かつ `seminar_session_id` が当月の行数 |
| `consultation_implementations_total` | int | `consultation_implemented_at` が当月の行数 |
| `contracts_total` | int | `contract_at` が当月の行数 |
| `revenue_total` | int | `contract_at` が当月の行の `contract_amount` 合計 |
| `cvr_click_to_register` | float (0-1) | `line_registrations / link_clicks` |
| `target_contracts` | int | 当月成約目標数（事前設定） |
| `gap` | int | `contracts_total - target_contracts` |
| `notes` | text | 月次振り返りメモ |

### 4.4 シートC：`ファネルKPIコホート`（CVR分析）

1コホート（LINE登録月）＝1行。前々月コホートの60日成績が確定したタイミングで集計。

| 列名 | 型 | 算出元 |
|---|---|---|
| `cohort_month` | text (YYYY-MM) | — |
| `registrations` | int | 当月の `registered_at` 行数 |
| `seminar_reservations_30d` | int | `seminar_reserved_at - registered_at <= 30日` の行数 |
| `seminar_attendances_30d` | int | `seminar_attended=TRUE` かつ 出席日−登録日 ≤ 30日 |
| `consultation_requests_30d` | int | `consultation_requested_at - registered_at <= 30日` |
| `consultation_implementations_30d` | int | `consultation_implemented_at - registered_at <= 30日` |
| `contracts_30d` | int | `contract_at - registered_at <= 30日` |
| `contracts_60d` | int | `contract_at - registered_at <= 60日`（遅延コンバージョン把握） |
| `revenue_30d` | int | 30日以内成約の `contract_amount` 合計 |
| `revenue_60d` | int | 60日以内成約の `contract_amount` 合計 |
| `cvr_register_to_seminar_attended` | float | `seminar_attendances_30d / registrations` |
| `cvr_seminar_to_consultation_implemented` | float | `consultation_implementations_30d / seminar_attendances_30d` |
| `cvr_consultation_to_contract` | float | `contracts_30d / consultation_implementations_30d` |
| `cvr_total_register_to_contract_30d` | float | `contracts_30d / registrations` |
| `cvr_total_register_to_contract_60d` | float | `contracts_60d / registrations` |

### 4.5 集計の自動／手動切り分け

| 項目 | 集計方法 | 工数 |
|---|---|---|
| 層1 SNS指標（impressions / reach / profile_visits / link_clicks） | `scripts/collect_metrics.py` を拡張してアカウントレベルInsightsを取得 → `ファネルKPI月次` に書き込み | 開発：中（数時間） |
| 層2 LINE登録 | LINE管理画面から登録通知を運用者が `ファネル登録ログ` に手動転記 | 運用：1件あたり30秒 |
| 層3 セミナー予約／出席 | Googleフォーム → スプレッドシート連携で自動取り込み（運用者が登録ログに転記） | 運用：1件あたり30秒 |
| 層4 個別相談／成約 | カレンダー＋手動記録 → 登録ログに転記 | 運用：1件あたり1〜2分 |
| `ファネルKPI月次` の算出列 | スプレッドシート関数（COUNTIFS / SUMIFS）で登録ログから自動集計 | 設計時のみ |
| `ファネルKPIコホート` の算出列 | スプレッドシート関数で登録ログから自動集計 | 設計時のみ |

**月次運用フロー**:
1. 月初：前月分の層1指標（Threads Insights）を `collect_metrics.py` で取得し `ファネルKPI月次` に書き込み
2. 月初：前月のLINE登録／セミナー／相談／成約イベントを運用者が登録ログに転記（リアルタイム転記が望ましいが、最低でも月次で確定）
3. 月初：`ファネルKPI月次` の算出列を確認（運用ダッシュボード）
4. 前々月コホートの60日成績が確定したら `ファネルKPIコホート` を確認（CVR検証）

---

## 5. 計測実装の現状と必要追加

### 5.1 現状の `scripts/collect_metrics.py`

投稿単位のインプレッション／いいね／返信／リポストを `メトリクスDB` に記録する仕組みは整備済み。

### 5.2 必要追加：アカウントレベルInsights取得

| 項目 | 取得対象 | API |
|---|---|---|
| 月間インプレッション | アカウント全体の総impressions | Threads Insights API（account-level metric） |
| 月間ユニーク到達者 | reach unique | Threads Insights API |
| 月間プロフィール訪問 | profile_views | Threads Insights API |
| 月間リンククリック | プロフィール内リンククリック数 | Threads Insights API（または短縮URL管理画面） |

`collect_metrics.py` に `collect_account_metrics(year_month)` 関数を追加し、月初cronで前月分を `ファネルKPI月次` に upsert する設計を想定。

### 5.3 必要追加：シート3つの新規作成

`scripts/sheets.py` に以下を追加することで Python 側からもアクセス可能になる:
- `get_or_create_funnel_log_sheet()`
- `append_funnel_log_row(record)`
- `get_funnel_monthly_kpi(year_month)` / `upsert_funnel_monthly_kpi(record)`

ただし運用初期は **手動入力で十分** に回せる規模（月数件〜数十件）なので、`sheets.py` 拡張は層1自動化（`collect_metrics.py` 連携）以外は後回しにできる。

### 5.4 短縮URLでクリック計測

Threads Insights の「リンククリック」はプロフィールリンク全体での集計で粒度が粗い。**Bitly等の短縮URL** を使って独立にクリック数を計測することで、層1-c（プロフィール訪問→リンククリック率）と層2（リンククリック→LINE登録率）の精度が大幅に上がる。

本ファネル開始時のセットアップ:
- LINE登録URL（例：`https://lin.ee/xxxxx`）を短縮URL（例：`https://bit.ly/yyyy`）でラップ
- ThreadsプロフィールおよびnoteプロフィールにはBitlyリンクを掲示
- 月初にBitly管理画面から月間クリック数を取得し `ファネルKPI月次` に記録

---

## 6. 立ち上げ時の作業リスト

| # | タスク | 担当 | 想定工数 |
|---|---|---|---|
| 1 | 公式LINE作成・基本設定 | 運用者 | 1時間 |
| 2 | 登録特典PDF「内側パターン書き出しワーク」作成 | 運用者＋Claude支援 | 4〜8時間 |
| 3 | 7日間ステップ配信の本文作成 | 運用者＋Claude支援 | 4〜8時間 |
| 4 | 短縮URLセットアップ（Bitly等） | 運用者 | 30分 |
| 5 | Threads／noteプロフィールに登録URL掲示 | 運用者 | 10分 |
| 6 | 無料webセミナーのスライド作成 | 運用者＋Claude支援 | 8〜12時間 |
| 7 | セミナー予約フォーム（Googleフォーム）作成 | 運用者 | 30分 |
| 8 | 個別相談予約フォーム＋カレンダー連携 | 運用者 | 1時間 |
| 9 | `ファネル登録ログ` ／ `ファネルKPI月次` ／ `ファネルKPIコホート` シート作成（スキーマ通り） | 運用者 | 1時間 |
| 10 | `collect_metrics.py` 拡張（層1アカウント指標取得） | 開発（後回し可） | 数時間 |
| 11 | 個別相談スクリプト作成 | 運用者＋Claude支援 | 2〜4時間 |

**最初の1ヶ月で必要な最低限**: #1〜#9（#10は手動Bitly確認で代替可）

---

## 7. 採用経緯（参考）

本A案は以下3案からの選定結果:

- A: 言語化ファネル（公式LINE中心）← **採用**
- B: コミュニティ駆動ファネル（オープンチャット中心）
- C: 未来設計ファネル（公式LINE＋診断起点）

**採用理由**:
- 1ヶ月リードタイムにB（コミュニティ）は短すぎる（コミュニティナーチャリングは通常3〜6ヶ月前提）
- 新規立ち上げ＋セミナー実績なしの状況でC（診断ボット）は初期工数が重く、検証が遅延する
- A は差別化軸「言語化＋書き換え」と各タッチポイントが地続きで、バックエンド契約までの物語の一貫性が3案中もっとも強い

B案は将来的に「既存購入者向けコミュニティ」または「バックエンド受講中の伴走コミュニティ」として、A案の後段に組み込むことを想定する。

---

## 8. 関連ファイル

- ポジショニング・ペルソナ・ファネル定義: `config/strategy.json`
- 発信者の事実情報: `docs/author_profile.md`
- SNS（Threads）の役割と既存運用: `CLAUDE.md`
- 既存メトリクス収集: `scripts/collect_metrics.py`
- Sheets ライブラリ: `scripts/sheets.py`
