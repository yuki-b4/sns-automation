...
- **テーマ選考の優先基準4項目**（一次感情への直結／夫側の実感の翻訳／賛否が割れる論点の許容／男女どちらが読んでも成立）は、実測の高パフォーマンス投稿（2026-08-08 の運用者観察：読者の約7割がルート1行目だけでフォローを判断、否定的な反応が着火剤になって閲覧数が伸びた、フォロワー男女比は約4:6）を反映して入れたもの。基準2には「夫にこう接すれば夫が動く」という**夫を操作する手順に着地させない**制約を明記してある。これは `strategy.json:positioning.differentiation` の『夫を動かすテクニックではなく自分の内面を磨く』を守るための歯止めなので、外すと差別化軸とバックエンド講座への導線が崩れる。
