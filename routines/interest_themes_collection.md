...
```
fetched_at | category | source | url | headline | summary | relevance_score | pain_point_id | priority_theme_id | angle_hint | used_in_post_id
```
...
| `pain_point_id` | 0〜3 or 空 | `strategy.json:persona.pain_points` のインデックス |
| `priority_theme_id` | `family_communication_design` 等 or 空 | `strategy.json:priority_themes.themes[].id` |
| `angle_hint` | 40 字以内 | `post_type × 切り口` を 1 案 |
| `used_in_post_id` | 空 | 後から運用者が手動で埋める枠（現状未使用） |
...
pain_point_id: 最も関連する pain_points のインデックス（0〜3）。該当なしは null
priority_theme_id: 該当する優先テーマ id。該当なしは null
...
fetched_at は実行時刻を JST ISO8601 で。used_in_post_id は常に空。pain_point_id / priority_theme_id が null のときは空セル。
...
- `used_in_post_id` カラムは将来の双方向連動のための予約枠
