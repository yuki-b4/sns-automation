...
```
fetched_at | category | source | url | headline | summary | relevance_score | pain_point | angle_hint
```
...
ステップ 1: ターゲット情報の取得
─────────────────────────────
GitHub MCP で yuki-b4/sns-automation リポジトリの config/strategy.json を取得し、以下 3 つを読み込む:
- persona.description（ターゲット像の全体像。職種・家族構成・志向などのコア属性）
- persona.pain_points（痛みの配列。全要素を保持）
- positioning.position / positioning.differentiation（誰向けに何を差別化しているか）

取得に失敗した場合は Slack に「❌ 失敗通知」を投げて即終了。

以降のすべての評価は、これら 3 つを常に参照しながら行うこと。persona.description は「どういう人向けか」を決める最重要前提、pain_points は「刺さるかどうか」の具体評価軸、positioning.differentiation は「スコア 5 足り得るかの分水嶺」として扱う。strategy.json の文面を勝手に解釈拡張せず、書かれている言葉に忠実に判断すること。
...
relevance_score（1〜5）
  5 = persona.pain_points のいずれかのど真ん中、かつ positioning.differentiation に直結する具体性がある
  4 = pain_points のいずれかに刺さる、または persona.description のコア属性（職種・家族状況・志向）で強く響く
...
summary: 120 字以内。「このターゲット（persona.description）にとって何が問題 or 機会か」の視点で書く。一般ニュース要約は禁止
...
fetched_at は実行時刻を JST ISO8601 で。pain_point が該当なしのときは空セル。
...
### `strategy.json` 更新時
`persona` / `positioning` を編集したら、本ルーチンも自動で追随する（GitHub MCP で毎回最新を読むため）。プロンプト側を書き換える必要はない
