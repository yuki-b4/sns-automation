...
def build_angle_matrix_section(combo: dict | None) -> str:
    ...
    従来は候補30項目を列挙していたが、1組に絞ることでトークン削減＋選択履歴をDB記録可能にする。"""
    if not combo:
        return ""
    return (
        "【今回のnoteで必ず使う切り口（Python側で事前選択済み・意味は差し替え禁止）】\n"
        f"- pain_point: {combo['pain_point']}\n"
        f"- 場面: {combo['situation']}\n"
        f"- 現れ方: {combo['manifestation']}\n"
        "※ この3要素の意味を保ったまま、タイトルか冒頭リード文（冒頭30字以内）で具体的な情景として必ず描く。\n"
        "※ ただし上記の候補文言（特に「場面」「現れ方」のラベル）はそのまま引用しない。"
        "毎回異なる自然な言い換えで表現する（例: 「リリース直前の週末」→「本番公開を控えた土曜の午後」／"
        "「月曜リリース前の日曜深夜」／「来週リリースを前にした土日」など、時間・場所・行動を具体化して"
        "毎回別のバリエーションにする）。同様に「集中力が切れる」「判断が雑になる」等もそのまま使わず、"
        "その日の情景に溶かし込んだ描写に言い換える。"
    )
...
- 【今回のnoteで必ず使う切り口】の pain_point / 場面 / 現れ方 の3要素は意味の差し替え禁止。ただしラベル文言は自然な表現に言い換え、タイトルか冒頭リード文で具体的な情景として必ず描く
...
- ペルソナは月〜金勤務・土日休みの平日勤務サラリーマン前提。時間表現（「平日」「終業後」「週末」「週明け」「金曜夜」など）はこの前提で使う（週末＝土日の休日、週明け＝月曜の始業、終業後＝平日の退勤後、家族時間が増える典型は平日夜と土日）。シフト勤務・土日出勤を常態とする想定や、平日休み前提の表現は避ける
- 【過去noteで扱った切り口】のタイトルに出てきた核概念語（例: 判断コスト・判断疲れ・金曜午後・集中力切れ）を、今回のタイトル・リード文・H2見出しのうち2箇所以上では核として使わない
- 【今回のnoteで必ず使う切り口】の pain_point / 場面 / 現れ方 の3要素は意味の差し替え禁止。ただしラベル文言は自然な表現に言い換え、タイトルか無料ゾーンのリード文で具体的な情景として必ず描く
