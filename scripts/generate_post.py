...
【補足リプライ】
（150〜250字。独身ITエンジニア・PM男性特化の視点で構造的理由・内側のパターンの視点を展開）"""
...
- 本文（ルート）で具体的な場面・体験・行動を描いた場合、そのあとに「実は」「つまり」「それは」「それ、〇〇ない」などの接続で自分の解釈・結論・オチを付け足さない。場面そのもので手を止めさせ、意味の受け取りは読者に委ねる（余白を残す）。悪い例：「彼女の前でスマホを握ってた／それ、心ここにあらず」→ 良い例：「彼女の前でスマホを握ってた」で止める
...
- 例：「多くの人が誤解しているパートナーとの距離感。残念ながら違います。実は、」

型B：ストーリー切断型（personal 適合、本文40〜80字）
- 一次経験ソース（妻との関係／クライアントの事例〔必ず「クライアントの〇〇さん」「知り合いに〇〇な人がいて」と明示区別〕／珈琲屋・エンジニア・PM・コーチのキャリア体験）から1つの場面を描き、感情・思考が動いた瞬間で切って次のリプライへ持ち越す
...
- 例：「なぜ自分が苦しくなる手前で必ず同じ場面を選んでしまうのか。」
{recent_posts_section}
{output_format}"""
