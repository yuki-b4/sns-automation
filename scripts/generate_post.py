...
- 続けて伏せた答えを開示し、「何をやめて何を始めるか」の具体行動に落とす。行動は「まずは〇〇から始めてみて。」の1アクションに絞るのが基本。ただしルートで型G（件数予告）または型F・型I（手段の名前を提示）を使った場合は、予告した件数どおり／その手段の中身として3〜5個の行動を並べてよい。この場合は最後に、順番の指定・前提の否定・添える態度のいずれかを1つだけ添えて手順を効かせる（例「①をやってから、という順番だけ間違えないこと」「ただし、必ず感謝は伝えてね」）。ルートで件数も手段名も出していないのに手順を長く列挙するのはしない
...
【本文】
（型A・型E・型F・型G・型H・型Iのいずれか。型A/型E/型F/型G/型I=20〜40字／型H=15〜35字。クリフハンガー／答えを示さない問い／件数予告／手段の名前の提示／逆説断定のいずれかで答えを伏せて終える。経験根拠は入れない）

【補足リプライ1】
（経験・立場の信頼根拠を一言→伏せた答え→思考・行動のクセ→行動へ。行動は「まずは〇〇から始めてみて。」の1アクションに絞るのが基本で、型G・型F・型Iでルートに件数か手段の名前を出した場合のみ3〜5個並べ、末尾に効かせる条件を1つ添える。150〜250字）
...
- 補足リプライ1で「なぜそのパターンが繰り返されるか」の理由を1〜2文で押さえ、そこから「何をやめて何を始めるか」の具体行動に落とす。行動は「まずは〇〇から始めてみて。」の1アクションに絞るのが基本。ただしルートで型G（件数予告）または型F・型I（手段の名前を提示）を使った場合は、予告した件数どおり／その手段の中身として3〜5個の行動を並べてよい。この場合は最後に、順番の指定・前提の否定・添える態度のいずれかを1つだけ添えて手順を効かせる（例「①をやってから、という順番だけ間違えないこと」「ただし、必ず感謝は伝えてね」）。ルートで件数も手段名も出していないのに手順を長く列挙するのはしない
...
【本文】
（型A・型C・型E・型F・型G・型H・型Iのいずれか。型A/型E/型F/型G/型I=20〜40字／型H=15〜35字／型C=80〜200字（見出し1行＋列挙最大5行・各15字以内）。手を止めるフックに徹する）

【補足リプライ1】
（型A=クリフハンガーの答え／型C=「では書き換えるには？」と逆側開示／型E=問いの答え／型F・型I=伏せた手段の中身を開示／型G=予告した件数どおりに開示／型H=断定の理由を開示。続けてなぜ繰り返すか→思考・行動のクセ→行動へ。行動は「まずは〇〇から始めてみて。」の1アクションに絞るのが基本で、型G・型F・型Iでルートに件数か手段の名前を出した場合のみ3〜5個並べ、末尾に効かせる条件を1つ添える。150〜250字）
...
【本文】
（型A・型E・型F・型G・型H・型Iのいずれか。型A/型E/型F/型G/型I=20〜40字／型H=15〜35字。意外性ある一言／逆説／答えを示さない問い／読者が得たい結果・避けたい状態の先置きで手を止める）

【補足リプライ】
（150〜250字。夫側の本音を翻訳する既婚男性視点で、夫の行動の理由・構造に言及。ルートで伏せたもの（クリフハンガーの答え／問いの答え／手段の中身／断定の理由）に必ず応える）"""
    elif post_type == "permission":
        type_specific_rules = ""
        output_format = """以下の形式で出力（説明・前置き不要）：

【本文】
（型A・型C・型F・型G・型H・型Iのいずれか。型A/型F/型G/型I=20〜40字／型H=15〜35字／型C=80〜200字（見出し1行＋列挙最大5行・各15字以内）。詳細・体験談はここに入れず補足リプライで展開）

【補足リプライ】
（150〜250字。型A=本文の一言に対する具体的な場面・体験・気づき・根拠を共感的に展開／型C=「では書き換えるには？」と逆側開示／型F・型I=伏せた手段の中身を開示／型G=予告した件数どおりに開示／型H=断定の理由を開示）"""
    else:
        type_specific_rules = ""
        output_format = """以下の形式で出力（説明・前置き不要）：

【本文】
（投稿タイプ（personal/dialogue）に応じ型カタログから選ぶ：personal=型B（40〜80字、ストーリー切断）／型D（60〜150字、同一人物の時系列対比）／型A（20〜40字）、dialogue=型E（20〜40字、答えを示さない問い）／型D（60〜150字）。両タイプとも型F・型G・型I（20〜40字）／型H（15〜35字）を選んでもよい。詳細・体験談は入れず、選んだ型に必要な要素だけをルートに）

【補足リプライ】
（150〜250字。選んだ型の開示：型B=ストーリーの続きと思考・行動のクセ／型D=何が変わったかの構造／型A=クリフハンガーの答え／型E=問いの答え／型F・型I=伏せた手段の中身を開示／型G=予告した件数どおりに開示／型H=断定の理由を開示。共感的に）"""
