...
- 本文（ルート）で具体的な場面・体験・行動を描いた場合、そのあとに「——」や「実は」「つまり」「それは」「それ、〇〇ない」などの接続で自分の解釈・結論・オチを付け足さない。場面そのもので手を止めさせ、意味の受け取りは読者に委ねる（余白を残す）。悪い例：「彼女の前でスマホを握ってた——それ、心ここにあらず」→ 良い例：「彼女の前でスマホを握ってた」で止める。ただし型カタログの型B（ストーリー切断型）を選んだ場合は、場面のあとに「そのとき自分の中で起きていたこと——」のような次のリプライへ持ち越す signal を意図的に置くため、このルールは適用しない
...
- signal 例：「そのとき、自分の中で起きていたこと——」「気づいたのは、その夜だった」「最初に出てきた感覚は、」
