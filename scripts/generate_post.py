...
- 続けて補足リプライ1で「実は〜」の答えを思考・行動のクセとして開示し、それを変えるための最初の一歩を「まずは〇〇から始めてみて。」の形でひとつの具体アクションに絞って示す（複数のステップを列挙しない）
...
【補足リプライ1】
（冒頭で経験・立場の信頼根拠を一言→「実は〜」の答え→思考・行動のクセ→それを変える最初の一歩を「まずは〇〇から始めてみて。」の形でひとつの具体アクションに絞って。150〜250字）
...
- 「気持ちを切り替える」「前向きに」「マインドセット」のような抽象的精神論だけで終わらない。具体的な「思考・行動のクセ」の認識・言語化・書き換え方法まで落とす
- 「再現性」「マインド」「思考・行動のクセ」「言語化」「書き換え」のいずれかを補足リプライ1に含める
- 構造の説明だけで終わらせず、補足リプライ1で示すひとつの具体アクションに添えて、自責解除／手放し許可のトーン（例：「このパターンに気づけたなら、もう自分を責めなくていい」「ここまで読めたあなたは、もう変わり始めている」）を1箇所だけ自然に組み込む。固定フレーズ化せず毎回違う表現にする
- 一般大衆向けの啓発トーンではなく、心理学・脳科学を本で学んできた読者がさらに自分の問題に応用できる専門的トーンで書く"""
            output_format = """以下の形式で出力してください（他の説明・前置き不要）：
...
- 述語契約①（目的語明示）: 各文で述語（動詞・形容詞）が「何を／誰を」対象にするかを文内に明示する。「考える」「言語化する」「動く」「感じる」「向き合う」など自動詞的に流せる動詞でも対象を必ず書く（×「言語化する」→○「苦しくなる場面のパターンを言語化する」、×「身体が感知している」→○「身体が夫との距離の変化を感知している」）
...
- 意外性のある一言＋「実は、」「残念ながら違います。実は、」などのクリフハンガーで終え、補足リプライ1で答えを開示する
- 例：「多くの女性が誤解している『夫が冷たくなる理由』。残念ながら違います。実は、」
