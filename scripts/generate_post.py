...
- 読者（既婚女性）が自分事化できる生活場面（家事・育児の合間、共働きの通勤・在宅、夫とのLINEや会話、寝室、記念日、週末の過ごし方等）は文脈に応じて使ってよい
...
- 述語契約①（目的語明示）: 各文で述語（動詞・形容詞）が「何を／誰を」対象にするかを文内に明示する。「考える」「言語化する」「動く」「感じる」「向き合う」など自動詞的に流せる動詞でも対象を必ず書く（×「言語化する」→○「苦しくなる場面のパターンを言語化する」、×「身体が感知している」→○「身体が座標のずれを感知している」）
...
- 属性別の対比（『結婚する人 vs しない人』『シゴデキ vs シゴデキナイ』等）は使わない。あくまで同一人物の時系列対比に限る
