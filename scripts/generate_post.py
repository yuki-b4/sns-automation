...
def build_prompt(strategy: dict, post_type: str, recent_posts: list[dict] | None = None) -> str:
    ...
    if post_type == "structure":
        if use_hook:
            type_specific_rules = """
【体系化系専用ルール（フック形式）】
- 本文（ルート）は1行・30〜60字に絞り、「多くの人が誤解している〇〇」「〇割の人が気づいていない〇〇」などの意外性のある一言＋「残念ながら違います。実は、」のようなクリフハンガーで終える。経験・立場の前置きはルートに入れず補足リプライ1の冒頭に回す
- 補足リプライ1の冒頭で、エンジニア・PM・コーチとしての経験や立場を一言で示す（例：「長年プロの開発者（エンジニア）として現場を見てきた経験から言える」）。具体的な年数は断定せず信頼の根拠として使う
- 続けて補足リプライ1で「実は〜」の答えを具体的な仕組み・設計ステップ（最低3項目）で開示する
...
【本文】
（1行・30〜60字。意外性のある一言＋クリフハンガーに絞る。例：「多くの人が誤解している〇〇——残念ながら違います。実は、」。経験根拠はここに入れない）

【補足リプライ1】
（冒頭で経験・立場の信頼根拠を一言→「実は〜」の答え→具体的な設計ステップ3〜5項目。150〜250字）
...
（読者への問いかけ、または再現性・持続性を強調する締めの一言。30〜60字）"""
            ...
            type_specific_rules = """
【体系化系専用ルール】
- 本文（ルート）は1行・30〜60字の意外性のある問題提起／常識を覆す一言で読者の手を止める
- 補足リプライ1で「なぜ意志力に頼ると限界があるか」のロジック→仕組み／設計プロセスを最低3ステップで具体的に示す（「毎朝〇時に」「〇分以内に」「3段階で」など数字・行動レベルで）
- 「心構え」「マインドセット」「根性」「精神力」だけで終わらない。具体的な行動・環境設計まで落とす
- 「再現性」「設計」「仕組み」「構造」のいずれかを補足リプライ1に含める
- 「できる人vsできない人」型の対比フォーマットは使わない（競合との差別化）
- 一般大衆向けの啓発トーンではなく、既に高い成果を出している人がさらに上へ行くための専門的トーンで書く
- 補足リプライ2の締めで、より深く知りたい読者向けに「プロフィールのnoteで更に詳しく書いている」旨を自然に入れてよい（投稿だけで完結する内容の場合は無理に入れず省略）。毎回異なる表現にし、固定フレーズ化を避ける
"""
...
【本文】
（1行・30〜60字。意外性のある問題提起／常識を覆す一言。読者の手を止めるフックに徹する）

【補足リプライ1】
（意志力の限界→仕組みが必要な理由→具体的な設計ステップ3〜5項目。150〜250字）
...
（読者への問いかけ、または再現性・持続性を強調する締めの一言。30〜60字）"""
        ...
        type_specific_rules = """
【業界考察系専用ルール】
- 本文（ルート）は1行・30〜60字の意外性／逆説で読者の手を止める。詳細な考察は補足リプライで展開する
- 補足リプライではハイパフォーマー（コンサル・PM・医師・弁護士・スタートアップ創業者）特化の視点で語る。一般向けの啓発トーンは禁止
...
【本文】
（1行・30〜60字。意外性のある一言／逆説で読者の手を止める）

【補足リプライ】
（150〜250字。ハイパフォーマー特化の専門的視点で構造的理由・設計視点を展開）"""
    elif post_type == "permission":
        type_specific_rules = """
【許可系専用ルール】
- 補足リプライの末尾で、投稿内容と地続きの流れで「プロフィールのnoteに置いている」旨を1行で添える。基本的に入れるが、投稿が単体で完結しすぎていて不自然になる場合は省略可。毎回異なる表現にし、固定フレーズ化を避ける（例：「同じような罪悪感を設計で解いた話、プロフィールのnoteに置いています」「ここから先の具体的な手順はプロフィールのnoteで」）。広告臭のある「詳しくはこちら」的な誘導はNG
"""
        output_format = """以下の形式で出力してください（他の説明・前置き不要）：

【本文】
（1行・30〜60字。意外性のある問題提起／常識を覆す一言。詳細・体験談・エピソードはここに入れず補足リプライで展開する）

【補足リプライ】
（150〜250字。本文の一言に対する具体的な場面・体験・設計・根拠を共感的に展開。末尾で投稿内容と地続きの流れでプロフィールのnoteへ1行の誘導を添える）"""
    else:
        type_specific_rules = ""
        output_format = """以下の形式で出力してください（他の説明・前置き不要）：
...
【補足リプライ】
（150〜250字。本文の一言に対する具体的な場面・体験・設計・根拠を共感的に展開）"""
...
{type_specific_rules}
【共通ルール】
- ルート（本文）：1行・30〜60字に圧縮し、意外性のある問題提起または常識を覆す一言に絞る。読者の手を止めて続きを読ませるフックに徹する（競合分析より、ルート短文＋リプライ1詳細展開の2投稿構成が最高ERを記録）
...
- 本文（ルート）で具体的な場面・体験・行動を描いた場合、そのあとに「——」や「実は」「つまり」「それは」「それ、〇〇ない」などの接続で自分の解釈・結論・オチを付け足さない。場面そのもので手を止めさせ、意味の受け取りは読者に委ねる（余白を残す）。悪い例：「家族の前でスマホを握ってた——それ、帰れてない」→ 良い例：「家族の前でスマホを握ってた」で止める
...
- 「〜だった」「ようやく気づいた」「〜けっこう衝撃だった」のような余韻系のエピソード締めフレーズや、「場面描写→気づき→教訓」の3段オチ構造を連続投稿で繰り返さない。締め方は断定／問いかけ／数値／情景止めなどを意識的に使い分ける
- 句読点・文末のリズムを整えすぎない。「〜なんだけど」「〜ですよね」のような文末の揺らし、言い淀み・訂正・具体的な固有名詞（日付・時刻・場所）を1投稿に1〜2箇所混ぜてよい。AI生成的な均質さよりも、一人称の生っぽさを優先する
...
- 子育ての具体エピソードは自分の体験として書かず、知り合い・クライアントの事例として書く（「クライアントの〇〇さん」「周囲の子育て中の仲間で」「よくある話で」）。夫婦（妻との関係）の実体験は語ってよい。家族時間のペインは多数の子育て世代を観察してきた立場から書く
