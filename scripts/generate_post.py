...
- 続けて補足リプライ1で「実は〜」の答えを具体的な内側のパターンと書き換えステップ（最低3項目）で開示する
...
【補足リプライ1】
（冒頭で経験・立場の信頼根拠を一言→「実は〜」の答え→具体的な内側のパターンと書き換えステップ3〜5項目。150〜250字）
...
- 補足リプライ1で「なぜ自分が苦しくなるパターンが繰り返されるか」のロジック→内側のパターンと書き換えプロセスを最低3ステップで具体的に示す（「毎日〇分」「〇回」「3段階で」など数字・行動レベルで）
- 「気持ちを切り替える」「前向きに」「マインドセット」のような抽象的精神論だけで終わらない。具体的な内側のパターン認識・言語化・書き換え方法まで落とす
- 「再現性」「マインド」「内側のパターン」「言語化」「書き換え」のいずれかを補足リプライ1に含める
- 構造の説明だけで終わらせず、書き換えステップの末項または補足リプライ2の締めに、自責解除／手放し許可のトーン（例：「このパターンに気づけたなら、もう自分を責めなくていい」「ここまで読めたあなたは、もう変わり始めている」）を1箇所だけ自然に組み込む。固定フレーズ化せず毎回違う表現にする
- 一般大衆向けの啓発トーンではなく、心理学・脳科学を本で学んできた読者がさらに自分の問題に応用できる専門的トーンで書く
- 補足リプライ2の締めで、より深く知りたい読者向けに「プロフィールのnoteで更に詳しく書いている」旨を自然に入れてよい（投稿だけで完結する内容の場合は無理に入れず省略）。毎回異なる表現にし、固定フレーズ化を避ける
"""
...
【補足リプライ1】
（型Aならクリフハンガーの答え／型Cなら「では書き換えるには？」と逆側を開示／型Eなら問いの答え。続けてなぜ繰り返すか→内側のパターン→具体的な書き換えステップ3〜5項目。150〜250字）
...
- 感情論・精神論で終わらず、構造的な理由・内側のパターンの視点で締める
"""
        output_format = """以下の形式で出力してください（他の説明・前置き不要）：
...
【補足リプライ】
（150〜250字。選んだ型に対応する開示：型Bならストーリーの続きと内側のパターン／型Dなら何が変わったかの構造／型Aならクリフハンガーの答え／型Eなら問いの答え。共感的に展開する）"""
...
- 比喩を断定で締めるAI的な「キレの良い一言」を避ける。「〇〇は△△の自動生成だった」「〇〇は結局、△△に過ぎなかった」のように比喩×言い切りでオチをつけるパターンは、文字面の鋭さで人工的な完成感が出る。比喩を使う場合は「〇〇みたいなものだったりする」「〇〇な気がする」「〇〇なんだよね」のような口語の余白を伴わせる
...
- 出力本文中で「内側のパターン」という語を書くときは、無修飾で使わず必ず読者所有を明示する修飾語を伴うこと。著者が読者に語る場面（author voice）は『あなたの内側のパターン』、読者の内面・気づきを描写する場面（reader voice）は『自分の内側のパターン』と書き分ける（修飾語なしの単独使用は『誰の』パターンか曖昧になり、読者の自分事化が弱まる）。なお、本ルールにおける『あなた』は出力本文を読む Threads の読者を指し、本プロンプトを受け取っているあなた（Claude）ではない
...
- 「自分が苦しくなる前にとりがちな行動」「うまくいかないときに自分がやってしまっている内側のパターン」など、片側だけを箇条書きで列挙し、補足リプライ1で「では、書き換えるには？」と逆側を開示する
