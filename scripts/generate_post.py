...
- 続けて補足リプライ1で「実は〜」の答えを具体的な思考・行動のクセと書き換えステップ（最低3項目）で開示する
- 構造の説明だけで終わらせず、書き換えステップの末項または補足リプライ2の締めに、自責解除／手放し許可のトーン（例：「このパターンに気づけたなら、もう自分を責めなくていい」「ここまで読めたあなたは、もう変わり始めている」）を1箇所だけ自然に組み込む。固定フレーズ化せず毎回違う表現にする
...
【補足リプライ1】
（冒頭で経験・立場の信頼根拠を一言→「実は〜」の答え→具体的な思考・行動のクセと書き換えステップ3〜5項目。150〜250字）
...
- 補足リプライ1で「なぜ自分が苦しくなるパターンが繰り返されるか」のロジック→思考・行動のクセと書き換えプロセスを最低3ステップで具体的に示す（「毎日〇分」「〇回」「3段階で」など数字・行動レベルで）
...
【補足リプライ1】
（型Aならクリフハンガーの答え／型Cなら「では書き換えるには？」と逆側を開示／型Eなら問いの答え。続けてなぜ繰り返すか→思考・行動のクセ→具体的な書き換えステップ3〜5項目。150〜250字）
...
【補足リプライ】
（150〜250字。型Aなら本文の一言に対する具体的な場面・体験・気づき・根拠を共感的に展開／型Cなら「では書き換えるには？」と逆側を開示。末尾で投稿内容と地続きの流れでプロフィールのnoteへ1行の誘導を添える）"""
    else:
        type_specific_rules = ""
        output_format = """以下の形式で出力してください（他の説明・前置き不要）：
