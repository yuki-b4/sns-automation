...
- 「能力の問題じゃなくて設計の問題」「才能じゃなく設計」のような決めフレーズを標語化して複数投稿で繰り返さないこと。同じ趣旨を伝える場合でも、一人称の実感ベース（例：「自分がサボりたいんじゃなくて、仕組みが休ませてくれないだけだった」）で毎回違う言い回しに書き換える
...
- 「〇〇をする」の形で動作を書く場合は、何を対象にするかが明確に伝わる目的語を含めること（「設計する」ではなく「判断の優先順位を設計する」、「仕組みをつくる」ではなく「退勤後の切り替え手順を仕組み化する」など）
