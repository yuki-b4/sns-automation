...
- クライアント・知り合いの事例を引く際は明示的に区別する（「クライアントの〇〇さん」「知り合いに〇〇な人がいて」のように示す）。夫婦（妻との関係）や自身の過去の恋愛経験は自分の体験として書ける
- 句読点・文末のリズムは整えすぎない。「〜なんだけど」「〜ですよね」のような揺らしを1〜2箇所混ぜてAI生成感を消す
- 述語契約①（目的語明示）: 各文で述語（動詞・形容詞）が「何を／誰を」対象にするかを文内に明示する。「考える」「動く」「感じる」「向き合う」など自動詞的に流せる動詞でも対象を必ず書く（×「身体が感知している」→○「身体が座標のずれを感知している」）
