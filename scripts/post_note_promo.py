...
- 爬虫類脳（生存・損失・即時報酬・社会的地位）に直撃する切り口を選ぶ：
  - 損失回避：このまま続けると失う具体的なもの（夫と自然体で愛し合える関係／素の自分らしさ／夫の反応に振り回されない心の安らぎ）を可視化する
...
- 否定型フック（「〇〇な人は△△だと思ってる。違う。」）や「テクニックじゃなくマインド」「外側じゃなく内側」のような決めフレーズは使わない
- 「IQが低い」「頭が悪い」のようなマイナス語での自己卑下は禁止。「天才ではない」「特別な才能はない」のように {{プラス語}}ではない の形で書く
- 数字は「30分以上／週3時間程度」のようにキリの良い値＋「以上／程度／前後」で丸める
- クライアント・知り合いの事例を引く際は明示的に区別する（「クライアントの〇〇さん」「知り合いに〇〇な人がいて」のように示す）。夫婦（妻との関係）や、自身が尽くすほど苦しくなっていた頃の経験は自分の体験として書ける
- 句読点・文末のリズムは整えすぎない。「〜なんだけど」「〜ですよね」のような揺らしを1〜2箇所混ぜてAI生成感を消す
- 述語契約①（目的語明示）: 各文で述語（動詞・形容詞）が「何を／誰を」対象にするかを文内に明示する。「考える」「動く」「感じる」「向き合う」など自動詞的に流せる動詞でも対象を必ず書く（×「身体が感知している」→○「身体が夫との距離の変化を感知している」）
