...
#
# spec は「設計骨格の2層（ideal_state / method_name）をどう1行に落とすか」の差として書く。
# 骨格導入前は型Fの spec が骨格そのものの言い換えになっていて二重定義だったため、
# 各型を骨格からの変換規則として表現し直してある（2026-08-12）。
# 「賛否が割れてよい」（旧・型H）はテーマ選考の優先基準3が持つので spec には書かない。
HOOK_TYPES = {
    "型F": {
        "name": "ベネフィット直球型",
        "spec": "ideal_state と method_name をそのまま1行に畳む。method_name は名前だけ出して中身は伏せる。20〜40字",
    },
    "型G": {
        "name": "件数予告型",
        "spec": "ideal_state を置き、method_name を「〇〇した方が良いこと3選」のように件数だけで予告する。件数は2〜5。20〜40字",
    },
    "型H": {
        "name": "逆説断定型",
        "spec": "「[method_name]な妻ほど[ideal_state]」のように、読者の常識と逆を向く短い断定で2層を畳む。理由は書かず断定だけで止める。15〜35字",
    },
    "型I": {
        "name": "状態提示切断型",
        "spec": "ideal_state を「〇〇な妻の特徴は」のように提示して体言止めで切り、method_name を伏せる。20〜40字",
    },
    "型B": {
        # 型名は generate_post.py の型カタログ（正本）と一字一句揃えること。
        # ここがズレると、運用者が本文執筆時に正本側で型を引けなくなる。
        "name": "ストーリー切断型",
        "spec": "ideal_state に向かう一次経験の場面（妻との関係／コーチとして向き合った場面）を1つ描き、感情・思考が動いた瞬間で切る。method_name は補足リプライ1で合流させる。40〜80字",
    },
}
...
【テーマ選考の優先基準（この順で効かせる）】
1. 一次感情への直結：読者が恐れる結果か欲しい結果（＝下の【投稿の設計骨格】第1層で言語化する状態）に、フック1行で接続できるテーマを優先する。心理メカニズムの名前（〇〇効果・〇〇回路・〇〇誤差）が主役になるテーマは、読者にとっての結果へ翻訳できない限り採らない。メカニズムは補足リプライで支える裏付けであって、テーマの看板にはしない
...
5. 読者のライフステージ特定性：結婚年数が積み上がったからこそ起きる場面を1つ以上含められるテーマを優先する。結婚10年前後（ペルソナの結婚歴の上半分）の生活を具体的な入り口にする。年数が浅くても深くても同じように成立する一般的な夫婦の場面（機嫌・会話・我慢）だけで組まず、「夫婦の会話が予定共有と業務連絡だけになった」「子どもの予定で1日が終わる」「二人で出かけた最後がいつか思い出せない」「実家や親の話が増えた」のように、その年数帯にしか出てこない場面へ落とす。ただし子どもが出てくる場面は読者側の生活としてのみ扱うこと（発信者に子どもはいないため、自分の一次経験として子育てを語らせない）
...
- 恐れの解消形（夫の心が離れない／浮気されない／このまま自分が消えない）でも獲得形（自然体のまま愛される／遠慮せず甘えられる／本音を言えるようになる）でもよい
...
- 発信者の一次経験・夫側の実感から出てくる手段にする
- 心理メカニズムの造語（〇〇効果・〇〇の法則）にはせず、何をするかが想像できる名前にする
...
- selection_note は{SELECTION_NOTE_MAX_LEN}字以内。上の選考基準1〜5のどれで読者を引っかける案なのかを明示する（例「基準1（恐れ）＋基準2で組んだ案」）
...
- {HOOK_PER_IDEA}案とも ideal_state と method_name から作る。カタログの各型は「2層をどう1行に落とすか」の違いなので、どの型を選んでも骨格から外れないこと
- 各案につき{HOOK_PER_IDEA}案、必ず異なる型で出す。type にはカタログの型名（例「ベネフィット直球型」）をそのまま書く
- {IDEA_COUNT}案全体で同じ型に寄せない。直近の使用回数が多い型は避け、使用が少ない型を優先的に当てる
- 逆説断定型は主張自体が賛否を割るので、使うのは選考基準3の枠（{IDEA_COUNT}案中1案）に収める
- 型の指定字数を守る。読者が1行目だけを見て「自分の話だ」と分かる具体語（夫・妻・浮気・本音・我慢・機嫌など読者の生活語）を必ず1つ以上入れる。さらに{IDEA_COUNT}案のうち1案以上は、その具体語を読者が自分の該当を判定できる属性語（結婚〇年目／子どもが小学校に上がってから／共働きに戻ってから 等の年数・ライフステージ語）にする。配信側からは読者の年齢・性別が見えないため、1行目で読者に自己申告させて当たりどころを判別する意図。全案に入れると入り口が単調になるので一部の案でよい
